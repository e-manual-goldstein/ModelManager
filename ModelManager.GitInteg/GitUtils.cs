...
namespace ModelManager.GitInteg
{
	public class GitUtils
	{
		...
		public static List<string> FindCommitsWithChanges(string repoName, DateTime fromDate, int? daysToRetrospect, int resultLimit = 20)
		{
			...
			{
				var branches = repo.Branches.Where(c => c.FriendlyName.ToUpper().Contains(repoName.ToUpper()));
				foreach (var branch in branches)
				{
					var commits = branch.Commits.Where(c => c.Committer.When >= fromDate).Take(resultLimit).ToList();
